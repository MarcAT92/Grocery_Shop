...
const ProductCard = ({ product }) => {
    ...
    return product && (
        ...
        } className="border border-primary-dull rounded-md md:px-4 px-3 py-2 bg-gray-100/50" >
                    ...
                    <div onClick={(e) => { e.stopPropagation(); }} className="text-primary">
                        {!cartItems[product._id] ? (
                            ...
                            <div className="flex items-center justify-center gap-1.5 md:gap-2 md:w-20 w-16 h-[34px] bg-primary/25 rounded select-none">
                                <button onClick={() => { removeFromCart(product._id) }} className="cursor-pointer text-md px-1.5 md:px-2 h-full" >
                                    -
                                </button>
                                <span className="w-5 text-center text-sm md:text-base">{cartItems[product._id]}</span>
                                <button onClick={() => { addToCart(product._id) }} className="cursor-pointer text-md px-1.5 md:px-2 h-full" >
                                    +
                                </button>
                            </div>
                        )}
